...
namespace API.Controllers
{
    ...
    public class RequirementController : ControllerBase
    {
        ...
        public IActionResult GetPrice(int requirementId, decimal? WeighOfMaterial, decimal? MachingningFee)
        {
            try
            {
                ...
                {
                    var MaterialPriceAtMoment = Math.Ceiling((decimal)(design.Material.Price * WeighOfMaterial));
                    var MasterGemStonePriceAtMoment = design.MasterGemstone != null ? design.MasterGemstone.Price : 0 ;
                    var StonePriceAtMoment = design.Stone != null ? design.Stone.Price : 0;
                    var MachiningFee = MachingningFee;
                    var TotalMoney = MaterialPriceAtMoment + MasterGemStonePriceAtMoment + StonePriceAtMoment + MachiningFee;
                    return Ok(new
                    {
                ...
                {
                    var MaterialPriceAtMoment = Math.Ceiling((decimal)(requirement.MaterialPriceAtMoment * requirement.WeightOfMaterial));
                    var MasterGemStonePriceAtMoment = requirement.MasterGemStonePriceAtMoment;
                    var StonePriceAtMoment = requirement.StonePriceAtMoment;
                    var MachiningFee = requirement.MachiningFee;
                    var TotalMoney = MaterialPriceAtMoment + MasterGemStonePriceAtMoment + StonePriceAtMoment + MachiningFee;
                    if (requirement.Status.Equals("4"))
                    {
                        ...
                        var anonymousRequirementDeposit = new
                        {
                            ...
                            MachiningFeeAnon = MachiningFee,
                            TotalMoneyAnon = TotalMoney,
                            Deposit = Math.Ceiling((decimal)(TotalMoney / 2))
                        };
                        return Ok(anonymousRequirementDeposit);
                    }
                    var paymentDeposit = _unitOfWork.PaymentRepository.Get(
                           x => x.RequirementsId == requirementId && x.Status.Equals("Paid")).FirstOrDefault();
                    var anonymousRequirement = new
                    {
                        ...
                        PayTheRest = TotalMoney - paymentDeposit.Amount
                    };
